...
export class PollService {
    ...
    async createPoll(userId: string, pollDto: CreatePollDto): Promise<IPoll> {
        try {
            const newPoll: IPoll = new this.pollModel({
                ...pollDto,
                user: userId,
            });
    ...
    async getUserPolls(userId: string): Promise<IPoll[]> {
        try {
            const polls: IPoll[] = await this.pollModel.find({ user: userId }).exec();
            return polls;
        } catch (e) {
            console.log(e);
            throw e;
